...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    // ?.then((movieList){
    //   debugPrint("now playing movie list api  ${movieList}");
    //
    //   setState(() {
    ...
    //
    // }).catchError((error){
    //   debugPrint("now playing movie list api error   ${error.toString()}");
    // });
    ...
    //   debugPrint(error.toString());
    // });
    /// get Datafrom Persistence

    // Now Playing Movies Database
    // mMovieModel.getNowPlayingMoviesFromDatabaseReactive()?.then((moviesList){
    //   debugPrint("now playing movie list database reactive  ${moviesList}");
    //   setState(() {
    //     mNowPlayingMovieList = moviesList;
    //   });
    // }).catchError((error){
    //   debugPrint("now playing movie list database error  ${error.toString()}");
    //
    // });
    //
    ...
    ///get data from Persistence with Reactive
    mMovieModel.getNowPlayingMoviesFromDatabaseReactive().listen((movielist) {
      // print("getNowPlayingMoviesFromDatabaseReactive movielist.length  ${movielist.length}");

      setState(() {
        mNowPlayingMovieList = movielist;
      });
    }).onError((error) {
      print("getNowPlayingMoviesFromDatabaseReactive error  ${error}");
    });

    mMovieModel.getPopularMoviesFromDatabaseReactive().listen((movielist) {
      print("getPopularMoviesFromDatabaseReactive movielist.length  ${movielist.length}");

      setState(() {
        mPopularMovieList = movielist;
      });
    }).onError((error) {
      // print("getPopularMoviesFromDatabaseReactive error  ${error}");
    });

    mMovieModel.getNowTopRelatedMoviesFromDatabaseReactive().listen((movielist) {
      // print("getNowTopRelatedMoviesFromDatabaseReactive movielist.length  ${movielist.length}");

      setState(() {
        mShowCaseMovieList = movielist;
        // mNowPlayingMovieList.addAll(movielist);
      });
    }).onError((error) {
      // print("getNowTopRelatedMoviesFromDatabaseReactive error  ${error}");
    });
    ...
    mMovieModel.getActors(1)
        ?.then((actors) {
          debugPrint("Actor list home page ${actors?.length}");
      setState(() {
        mActors = actors ;
      });
    }).catchError((error) {
      debugPrint("Actor list home page error ${error.toString()}");
    });
